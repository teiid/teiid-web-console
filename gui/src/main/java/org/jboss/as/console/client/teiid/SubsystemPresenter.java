...
public class SubsystemPresenter extends
	...
	public interface MyView extends View {
		...
		void setLogHandlerStatus(String context, String name,
				boolean dbAppender, boolean exists);
	...
	public SubsystemPresenter(EventBus eventBus, MyView view, MyProxy proxy,
		...
		this.configurationEntityAdapter = new EntityAdapter<>(
				SubsystemConfiguration.class, metadata);
		this.loggerAdaptor = new EntityAdapter<TeiidLogger>(TeiidLogger.class,
				metadata);
		this.transportEntityAdapter = new EntityAdapter<Transport>(
				Transport.class, metadata);
		...
		dispatcher.execute(new DMRAction(operation),
				new SimpleCallback<DMRResponse>() {
					...
					public void onSuccess(DMRResponse dmrResponse) {
						...
						SubsystemConfiguration bean = configurationEntityAdapter
	...
	public void save(SubsystemConfiguration t, Map<String, Object> changeset) {
		...
		ModelNode operation = this.configurationEntityAdapter.fromChangeset(
				changeset, address);
	...
	public void getLogHandlerStatus(final String context) {
		...
		this.dispatcher.execute(new DMRAction(operation),
				new SimpleCallback<DMRResponse>() {
					@Override
					public void onSuccess(DMRResponse result) {
						ModelNode response = result.get();
						if (isSucess(response)) {
							if (response.get(RESULT).isDefined()) {
								TeiidLogger logger = loggerAdaptor
										.fromDMR(response.get(RESULT));
	...
	public void checkLogHandlerStatus(final String context, final String name,
			final boolean dbAppender) {
		ModelNode address = RuntimeBaseAddress.get();
		address.add("subsystem", "logging");
		if (dbAppender) {
		...
		this.dispatcher.execute(new DMRAction(operation),
				new SimpleCallback<DMRResponse>() {
					...
					public void onSuccess(DMRResponse result) {
						ModelNode response = result.get();
						if (isSucess(response)) {
							getView().setLogHandlerStatus(context, name,
									dbAppender, true);
						}
					}
	...
	public void addOrRemoveLogger(final String context,
			final boolean addOperation) {
		ModelNode address = RuntimeBaseAddress.get();
		address.add("subsystem", "logging");
		address.add("logger", context);
		ModelNode operation = new ModelNode();
	...
	public void addFileHandler(final String name, String fileName) {
		...
		operation
				.get("formatter")
				.set(new ModelNode()
						.set("%d{HH:mm:ss,SSS} %-5p [%c] (%t) %X{teiid-session} %s%E%n"));

		ModelNode file = new ModelNode();
		file.get("path").set(new ModelNode().set(fileName));
		file.get("relative-to")
				.set(new ModelNode().set("jboss.server.log.dir"));
	...
	private boolean isSucess(ModelNode response) {
		return response.hasDefined("outcome")
				&& response.get("outcome").asString().equals("success");
	}
	...
	public void removeFileHandler(final String name) {
		...
		this.dispatcher.execute(new DMRAction(operation),
				new SimpleCallback<DMRResponse>() {
					...
					@Override
					public void onFailure(Throwable caught) {
						Console.error("Failed to remove log handler " + name,
								caught.getMessage());
					}
				});
	}
	...
	public void addLogger(final String context, String level, String handler) {
		// subsystem=logging/logger=com.your.package.name1:add(level=DEBUG,
		// handlers=[foo])
		ModelNode address = RuntimeBaseAddress.get();
		address.add("subsystem", "logging");
		address.add("logger", context);
	...
	public void removeLogger(final String context) {
		...
		this.dispatcher.execute(new DMRAction(operation),
				new SimpleCallback<DMRResponse>() {
					...
					@Override
					public void onFailure(Throwable caught) {
						Console.error("Failed to remove logger " + context,
								caught.getMessage());
					}
				});
	}
		...
		this.dispatcher.execute(new DMRAction(operation),
				new SimpleCallback<DMRResponse>() {
					...
					public void onSuccess(DMRResponse dmrResponse) {
						...
						List<Property> children = response.get(RESULT)
								.asPropertyList();
						...
						for (Property child : children) {
							...
							Transport transport = transportEntityAdapter
									.fromDMR(model);
					...
					public void onFailure(Throwable caught) {
						Console.error(
								"Failed to retrieve configuration for Teiid transports",
								caught.getMessage());
					}
				});
	}
				...
				new SimpleCallback<DMRResponse>() {
					...
					public void onSuccess(DMRResponse dmrResponse) {
						...
						if (ModelNodeUtil.indicatesSuccess(result)) {
							Console.info(Console.MESSAGES
									.added("Teiid Transport "
											+ transport.getName()));
						} else {
							Console.error(Console.MESSAGES
									.addingFailed("Teiid Transport "
											+ transport.getName()), result
									.toString());
						}
						loadTransports();
					}
	...
	public void saveTransport(final Transport transport,
		...
		ModelNode operation = this.transportEntityAdapter.fromChangeset(
				changeset, address);
		...
		dispatcher.execute(new DMRAction(operation),
				new SimpleCallback<DMRResponse>() {
					...
					public void onSuccess(DMRResponse result) {
						...
						if (success) {
							Console.info(Console.MESSAGES
									.saved("Teiid Transport "
											+ transport.getName()));
						} else {
							Console.error(Console.MESSAGES
									.saveFailed("Teiid Transport "
											+ transport.getName()), response
		...
		dispatcher.execute(new DMRAction(operation),
				new SimpleCallback<DMRResponse>() {
					...
					public void onSuccess(DMRResponse dmrResponse) {
						...
						if (ModelNodeUtil.indicatesSuccess(result)) {
							Console.info(Console.MESSAGES
									.deleted("Teiid Transport "
											+ transport.getName()));
						} else {
							Console.error(Console.MESSAGES
									.deletionFailed("Teiid Transport "
											+ transport.getName()), result
									.toString());
						}
						loadTransports();
				...
				new SimpleCallback<DMRResponse>() {
					...
					public void onSuccess(DMRResponse dmrResponse) {
						...
						for (Property child : children) {
							...
							Translator translator = translatorAdapter
									.fromDMR(model);
					...
					public void onFailure(Throwable caught) {
						Console.error(
								"Failed to retrieve configuration for Teiid translators",
								caught.getMessage());
					}
				});
	}

	public void saveTranslator(final Translator translator,
			Map<String, Object> changeset) {
		...
		ModelNode operation = this.translatorAdapter.fromChangeset(changeset,
				address);
		...
		dispatcher.execute(new DMRAction(operation),
				new SimpleCallback<DMRResponse>() {
					...
					public void onSuccess(DMRResponse result) {
						...
						if (success) {
							Console.info(Console.MESSAGES
									.saved("Teiid Translator "
											+ translator.getName()));
						} else {
							Console.error(Console.MESSAGES
									.saveFailed("Teiid Translator "
											+ translator.getName()), response
									.getFailureDescription());
						}
						loadTranslators();
					}
		...
		dispatcher.execute(new DMRAction(operation),
				new SimpleCallback<DMRResponse>() {
					...
					public void onSuccess(DMRResponse dmrResponse) {
						...
						if (ModelNodeUtil.indicatesSuccess(result)) {
							Console.info(Console.MESSAGES
									.deleted("Teiid Translator "
											+ translator.getName()));
						} else {
							Console.error(Console.MESSAGES
									.deletionFailed("Teiid Translator "
											+ translator.getName()), result
									.toString());
						}
						loadTranslators();
					}
		...
		dispatcher.execute(new DMRAction(operation),
				new SimpleCallback<DMRResponse>() {
					...
					public void onSuccess(DMRResponse dmrResponse) {
						...
						if (ModelNodeUtil.indicatesSuccess(result)) {
							Console.info(Console.MESSAGES
									.added("Teiid Translator "
											+ translator.getName()));
						} else {
							Console.error(Console.MESSAGES
									.addingFailed("Teiid Translator "
											+ translator.getName()), result
									.toString());
						}
						loadTranslators();
