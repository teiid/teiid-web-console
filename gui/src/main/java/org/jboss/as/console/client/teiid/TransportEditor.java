...
public class TransportEditor implements Persistable<Transport> {
	...
	public Widget asWidget() {
		...
		ClickHandler deleteClickHandler = new ClickHandler() {
			...
			public void onClick(ClickEvent event) {
				...
				Feedback.confirm(
						...
						Console.MESSAGES.deleteConfirm("Transport "
								+ selection.getName()),
		...
		ToolButton deleteBtn = new ToolButton(
		...
		this.table = new DefaultCellTable<Transport>(7,
				new ProvidesKey<Transport>() {
					@Override
		...
		MultipleToOneLayout layoutBuilder = new MultipleToOneLayout()
				...
				.setDescription(
						new SafeHtmlBuilder()
								.appendEscaped(
										"Transport provides a mechanism to "
												+ "connect to Teiid, For ex: jdbc, odbc connections")
								.toSafeHtml())
	...
	static List<FormItem<?>> buildCommonFormItems() {
		...
		TextBoxItem socketBinding = new TextBoxItem("socketBinding",
				"Socket Binding Name (refers to port)");

		NumberBoxItem maxSocketThreads = new NumberBoxItem("maxSocketThreads",
				"Max Socket Threads");

		// maxSocketThreads.setValue(0);

		// TextBoxItem authenticationDomain = new
		// TextBoxItem("authenticationDomain",
	...
	static List<FormItem<?>> buildSSLFormItems() {
		...
		TextBoxItem keyManagementAlgorithm = new TextBoxItem(
				"keyManagementAlgorithm", "Key Management Algorithm");

		TextBoxItem enabledCipherSuites = new TextBoxItem(
				"enabledCipherSuites", "Enabled Cipher Suites");

		TextBoxItem keystoreName = new TextBoxItem("keystoreName",
				"Keystore Name");
		TextBoxItem keystorePassword = new TextBoxItem("keystorePassword",
				"Keystore Password");
		TextBoxItem keystoreType = new TextBoxItem("keystoreType",
				"Keystore Type");

		CheckBoxItem truststoreCheckExpired = new CheckBoxItem(
				"truststoreCheckExpired", "Truststore Check Expired");
		truststoreCheckExpired.setValue(false);

		TextBoxItem truststoreName = new TextBoxItem("truststoreName",
				"Truststore Name");
		TextBoxItem truststorePassword = new TextBoxItem("truststorePassword",
				"Truststore Password");

		return Arrays.asList(sslEnabled, sslMode, sslAuthMode, sslProtocol,
				keyManagementAlgorithm, enabledCipherSuites, keystoreName,
				keystorePassword, keystoreType, truststoreCheckExpired,
	...
	private Transport getCurrentSelection() {
		return ((SingleSelectionModel<Transport>) this.table
	...
	public void launchTransportWizard() {
		try {
			this.window = new DefaultWindow(
					Console.MESSAGES.createTitle("Transport"));
